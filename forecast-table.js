const API_URL =
  "https://5wqz99uez1.execute-api.ap-southeast-2.amazonaws.com/v1/api/aqi/forecast";
...
function renderTable(data, dateList) {
      ...
      .map((date) => {
        const dateParts =
          typeof date === "string" && date.includes("-")
            ? date.split("-")
            : ["0000", "00", "00"];
        const dateLabel = `${dateParts[1]}/${dateParts[2]} ${getDayOfWeek(
          date
        )}`;
  ...
  subHeaderRow.innerHTML =
    `<th></th>` + dateList.map(() => `<th>AQI</th><th>污染物</th>`).join("");
    ...
    tr.innerHTML =
      ...
      dateList
        .map((date) => {
          ...
          if (item) {
            return `<td class="${getAQIClass(item.aqi)}">${item.aqi}</td><td>${
              item.majorpollutant || "-"
            }</td>`;
          } else {
